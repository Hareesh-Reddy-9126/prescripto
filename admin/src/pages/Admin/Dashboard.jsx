  ...
  if (!aToken) {
    return (
      <div className='m-5 rounded border border-dashed border-slate-200 bg-white p-6 text-sm text-slate-500'>
        Sign in as an administrator to view dashboard insights.
      </div>
    )
  }
        ...
        <div className='pt-4 border border-t-0'>
          ...
          {latestAppointments.map((item, index) => (
            <div className='flex items-center px-6 py-3 gap-3 hover:bg-gray-100' key={index}>
              ...
              {item.cancelled ? <p className='text-red-400 text-xs font-medium'>Cancelled</p> : item.isCompleted ? <p className='text-green-500 text-xs font-medium'>Completed</p> : <img onClick={() => cancelAppointment(item._id)} className='w-10 cursor-pointer' src={assets.cancel_icon} alt="Cancel appointment" />}
            </div>
          ))}
        </div>
      </div>
